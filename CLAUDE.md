...
# Start dev server (auto-reload is not configured; restart after code changes if needed)
python server.py
# → http://localhost:8000
...
No build command, linter, type checker, test runner, or single-test command is configured. The server mounts `static/` at `/static` and redirects `/` to `/static/index.html`.
...
2. **Frontend pre-fetches city data before AI planning**: `generatePlan()` calls `fetchCityData()` for each city, then sends `destinations[]`, `city_data[]`, preferences, transport choices, and `start_date` to `/api/plan`. Backend planning expects `city_data[]` to contain `{city, pois[], center{lat,lng}, days}`.
...
7. **Single mutable frontend state object**: All frontend state lives in the `state` object at the top of `static/app.js`. `applyPlan()` is the single hydration point — it transforms the raw API response via `mapPlanToItems()` (which injects transport/food/hotel pseudo-items into each day) and stores it as `state.itinerary`. Render functions read from `state`.

8. **Per-city days control (not global slider)**: Each city in `state.cities` carries its own `days` field (1–7). `cityDayMap()` expands the itinerary day-by-day per city's count; `computeTotalDays()` sums them. The global days display is derived, not the source of truth. When calling `/api/plan`, both `destinations[].days` and `city_data[].days` carry each city's actual day count.

9. **Station data caching**: On startup, `services/train_service.py` downloads the 12306 station list and caches it to `services/.cache/station_map.json` (7-day TTL). If the download fails, it falls back to `BUILTIN_STATION_MAP`. `services/.cache/` is ignored by git.

10. **Ignored design/legacy artifacts may exist locally**: `static/index-before-ui-rebuild.html` and `static/index.html.artifact.json` may be present as local ignored artifacts. Do not rely on them as tracked source files, and do not hand-edit the artifact JSON.
...
| `static/app.js` | Frontend state, city/day controls, backend POI fetching, map rendering, itinerary timeline, AI→fallback generation |
| `static/styles.css` | Warm terracotta theme, responsive 3-column layout, mobile panes |
...
- `.conda/`, `.env`, `services/.cache/`, `*.artifact.json`, and `static/index-before-ui-rebuild.html` are ignored. Avoid treating ignored local artifacts as source of truth.
