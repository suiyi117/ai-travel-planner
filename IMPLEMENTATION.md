...
- 用 MapLibre GL Leaflet + OpenFreeMap 矢量底图解决高分屏下栅格地图“清晰但字小 / 字大但模糊”的冲突。
- 保留 Leaflet 的标记、路线、弹窗、缩放和拖动交互；矢量资源不可用时自动保留原高德栅格底图。
- 在地图边界加入 GCJ-02 与 WGS84 双向转换，保持高德 POI、道路路线和地图点选 API 的坐标契约不变。
